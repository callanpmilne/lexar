<?php

require_once('../src/class/Entities/EntityType.php');
require_once('../src/methods/Entity/createEntityType.php');

/**
 * Create Entity Type Attribute Page
 */

$isCreateEntityTypeAttributeSubmit = 
  array_key_exists('is_create_entity_type_attribute_submit', $_POST)
    && '1' === $_POST['is_create_entity_type_attribute_submit'];

if (0 && $isCreateEntityTypeAttributeSubmit) {
  
  // Entity ID
  $entityID = $_POST['uuid'];

  // Entity Name
  $entityName = $_POST['entityName'];

  // attempt to create entity
  $result = createEntity(new Entity(
    $entityID,
    $entityName
  ));

  // redirect user to entity admin page on successful creation
  if (true === $result) {
    ?>
    <script>"use strict"; (function (w) {
      const entityURI = '/admin/view/entity/<?=$entityID?>';
      w.location.assign(entityURI);
    })(window);</script>
    <?php
    exit(0);
  }

}
?>

<main>
  <div id="PageTitle">
    <h1>Create Entity Type Attribute</h1>

    <p class="breadcrumbs">
      <a href="/admin">
        &larr; Admin Dashboard
      </a>
    </p>
  </div>

  <form 
    action="/admin/create/entity/type/attribute" 
    method="POST">
    <?php include('../src/common/form/create/entity/type/attribute.php'); ?>
  </form>

  <?php if ($isCreateEntityTypeAttributeSubmit) : ?>
    <div
      class="component-form-login-debug">
      <h2>Debug</h2>
      <pre><?php var_dump($_POST); ?></pre>
    </div>
  <?php endif; ?>
</main>

<style>
  div.component-form-login-debug {
    border: 1px solid;
    padding: 0 1rem 1rem;
    margin-bottom: 2rem;
  }
</style>